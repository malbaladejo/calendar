...
export class CustomLabelsService {
  ...
  public async setLabelAsync(date: Date, label: string): Promise<CustomLabel> {
    let labelItem = this.getItemOrDefault(date);
    labelItem.label = label;
    return await this.saveItemAsync(labelItem);
  }
  ...
  private async saveItemAsync(item: CustomLabel): Promise<CustomLabel> {
    try {
      const year = item.date.getFullYear();
      const result = await this.customLabelsDataService.saveDataAsync(year, item);
      if (result) {
        item = result;
      }
